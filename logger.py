...
class LogFormatter(logging.Formatter):
    ...
    formats = {
        ...
        logging.CRITICAL: light_blue + '[%(asctime)s]' + reset + ' - ' + bold_red + '%(levelname)s' + reset +
        ' - %(processName)s - %(threadName)s - (%(filename)s:%(lineno)d) : ' +
        reset + purple + '%(name)s' + reset + ' - ' +
        bold_red + '%(message)s' + reset
    }

    def format(self, record):
        formatter = logging.Formatter(
            self.formats[record.levelno], datefmt='%d.%b.%Y %H:%M:%S')
